...
namespace gameplay {
    ...
    void PlayerHUDSystem::render(our::World* world, our::Entity* playerEntity, glm::ivec2 windowSize,
        ...
        glm::mat4 outlineTransform =
            glm::translate(glm::mat4(1.0f), glm::vec3(barPos.x - outlineThickness, barPos.y - outlineThickness, 0.0f)) *
            glm::scale(glm::mat4(1.0f), glm::vec3(healthBarRect.size.x + outlineThickness * 2,
                                                  healthBarRect.size.y + outlineThickness * 2, 1.0f));
        ...
        if (healthPercentage > 0.0f) {
            glm::mat4 healthTransform = glm::translate(glm::mat4(1.0f), glm::vec3(barPos.x, barPos.y, 0.0f)) *
                                        glm::scale(glm::mat4(1.0f), glm::vec3(healthBarRect.size.x * healthPercentage,
                                                                              healthBarRect.size.y, 1.0f));
        ...
        if (textRenderer) {
            ...
            our::UIRect tr = ammoTextRect;
            tr.offset += glm::vec2(outlineSpread, outlineSpread);
            textRenderer->drawText(&testFont, ammoText, tr, windowSize, textScale, orthoVP, outlineColor);

            our::UIRect bl = ammoTextRect;
            bl.offset += glm::vec2(-outlineSpread, -outlineSpread);
            textRenderer->drawText(&testFont, ammoText, bl, windowSize, textScale, orthoVP, outlineColor);

            our::UIRect tl = ammoTextRect;
            tl.offset += glm::vec2(-outlineSpread, outlineSpread);
            textRenderer->drawText(&testFont, ammoText, tl, windowSize, textScale, orthoVP, outlineColor);

            our::UIRect br = ammoTextRect;
            br.offset += glm::vec2(outlineSpread, -outlineSpread);
