...
namespace our::texture_utils {
    ...
    Texture2D* loadImage(const std::string& filename, bool srgb = false, bool generate_mipmap = true);
    // This function loads an image from an in-memory buffer and sends its data to a Texture2D
    Texture2D* loadImageFromMemory(const unsigned char* data, std::size_t size, bool srgb = true,
